      ...
      <div className="grid gap-5 lg:grid-cols-[minmax(0,1.05fr)_minmax(320px,0.95fr)] lg:items-start">
        <section className="rounded-2xl border border-zinc-800 bg-zinc-900/70 p-4 sm:p-6" aria-labelledby="calculator-inputs">
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            <div>
              <label htmlFor="current-tier" className="mb-2 block text-sm font-medium text-zinc-300">Current tier</label>
              <select id="current-tier" value={currentTier} onChange={(e) => handleCurrentTier(Number(e.target.value))} className="min-h-12 w-full rounded-lg border border-zinc-700 bg-zinc-950 px-3 text-base text-white focus:border-amber-500 focus:outline-none focus:ring-2 focus:ring-amber-500/20">
                {pickaxes.map((pickaxe) => <option key={pickaxe.id} value={pickaxe.tier}>Tier {pickaxe.tier} — {pickaxe.name}</option>)}
              </select>
            </div>
            <div>
            ...
            <div>
              <label htmlFor="target-tier" className="mb-2 block text-sm font-medium text-zinc-300">Target tier</label>
              <select id="target-tier" value={targetTier} onChange={(e) => setTargetTier(Number(e.target.value))} className="min-h-12 w-full rounded-lg border border-zinc-700 bg-zinc-950 px-3 text-base text-white focus:border-amber-500 focus:outline-none focus:ring-2 focus:ring-amber-500/20">
                {pickaxes.filter((pickaxe) => pickaxe.tier >= currentTier).map((pickaxe) => <option key={pickaxe.id} value={pickaxe.tier}>Tier {pickaxe.tier} — {pickaxe.name}</option>)}
              </select>
            </div>
            <div>
